...
계산과학(scientific computing)은 전통적으로 최고의 성능을 요구하지만, 여전히 대부분의 전문가들은 상대적으로 느린 동적 언어를 이용하여 업무를 처리하고 있다.
계산과학에 동적 언어를 즐겨쓰는 여러가지 이유로 보건데, 이러한 추세가 쉽게 줄어들 것으로는 보이지 않는다.
다행히 근래의 언어 디자인과 컴파일러 기법의 발달은, 미뤄두었던 성능 문제를 해결함으로서
프로토타이핑 작업에 요구되는 개별 환경의 생산성과, 성능이 중요한 애플리케이션의 구축에서 그 효용을 충분히 발휘한다.
줄리아 프로그래밍 언어는 다음과 같은 역할을 수행한다. 기존의 정적 타입 언어에 견줄만한 성능을 갖춘 유연한 동적 언어(과학과 수학 분야 컴퓨팅에 적합한).

줄리아 컴파일러는 Python이나 R과 같은 언어의 인터프리터 활용 방법과 다르기 때문에, 처음에는 줄리아가 그리 직관적이지 않다고 느낄 것이다.
만약에 작성한 코드가 느리다면 다른 것보다도 [성능 팁](@ref man-performance-tips)을 읽어보길 권한다.
한 번이라도 줄리아가 어떤 식으로 작동하는지 이해한 뒤라면, C에 근접하는 속도로 손쉽게 코드를 작성할 수 있다.
...
기존에 있는 동적 언어와 비교해 보는 줄리아만의 독특한 점은:
  ...
  * 핵심 언어는 최소한으로 꾸린다; 정수를 다루는 프리미티브 연산자(`+` `-` `*` 같은)를 포함하는, 줄리아 Base와 표준 라이브러리는 줄리아 코드로 짜여짐
  * 객체를 구성하고 서술하는 타입(types)을 풍부하게 지원하며, 타입 선언을 만들 때도 선택적으로 사용됨
...
보통의 프로그래머라면 개의치 않을 타입과 멀티플 디스패치가 줄리아의 핵심 개념이다: 함수들은 서로 다른 인자 타입들을 조합함으로서 정의되고
...
사용하기 편하면서도 강력하고 효율적인 언어를 줄리아는 목표하고 있다. 다른 시스템과 견주어 줄리아를 씀으로 좋은 점은 다음과 같다:
  ...
  * 자유롭게 사용 가능하며 오픈 소스 ([MIT 라이센스](https://github.com/JuliaLang/julia/blob/master/LICENSE.md))
  * 사용자가 정의한 타입 또한 내장한 타입처럼 빠르고 간결함
  * 성능을 위해 코드를 벡터화할 필요가 없음(벡터화하지 않은 코드도 빠르다)
  * 병렬과 분산 처리를 위해 디자인됨
